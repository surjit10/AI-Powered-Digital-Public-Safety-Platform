...
export function useAuth() {
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      // MFA required — the backend returns mfa_required: true
      ...
      if (payload.role !== 'TELECOM_ADMIN') {
        throw new Error(`Access denied. This portal requires the TELECOM_ADMIN role, but your account has the '${payload.role}' role.`);
      }
